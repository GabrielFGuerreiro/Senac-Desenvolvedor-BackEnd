...
CREATE PROCEDURE SP_INSERT_ALUGUEL
(
	IN _CD_CLIENTE INT,
	IN _CD_FUNCIONARIO INT,
	IN _DT_RETIRADA DATETIME,
	IN _DT_DEVOLUCAO DATETIME,
	IN _PAGO BIT,
	IN _FORMA_PAGAMENTO VARCHAR(50),
	IN _QNT_VEZES INT,
	IN _CD_EQUIPAMENTO INT,
	IN _QTD INT
)
BEGIN
	...
	DECLARE
	@VL_PAGO = NULL,
	@CD_ALUGUEL = LAST_INSERT_ID();
	@VL_UNITARIO = (SELECT VL_HORA FROM equipamento WHERE CD_EQUIPAMENTO = _CD_EQUIPAMENTO);
	@VL_TOTAL = @VL_ITEM * _QTD;
	
	CASE WHEN PAGO = 1
	THEN @VL_PAGO = @VL_TOTAL
	ELSE @VL_PAGO = null
	
	INSERT INTO aluguel 
	(CD_CLIENTE, CD_FUNCIONARIO, DT_RETIRADA, DT_DEVOLUCAO, VL_A_PAGAR, VL_PAGO, PAGO, FORMA_PAGAMENTO, QNT_VEZES)
	VALUES
	(_CD_CLIENTE, _CD_FUNCIONARIO, _DT_RETIRADA, _DT_DEVOLUCAO, @VL_TOTAL, @VL_PAGO, _PAGO, _FORMA_PAGAMENTO, _QNT_VEZES)
	...
	VALUES
	(_CD_EQUIPAMENTO, @CD_ALUGUEL, @VL_TOTAL, @VL_UNITARIO, _QTD);

END 

DELIMITER ;

CALL SP_INSERT_ALUGUEL(3,1 , '2026-03-29 09:00:00', NULL, , NULL, 0, NULL, NULL, 2, 4)
SELECT * FROM aluguel 11
SELECT * FROM aluguel_equipamento 18
SELECT * FROM equipamento 86

DROP PROCEDURE SP_INSERT_ALUGUEL
