...
BEGIN
	SET @VL_UNITARIO = (SELECT VL_HORA FROM equipamento WHERE CD_EQUIPAMENTO = _CD_EQUIPAMENTO);
	SET @VL_TOTAL = @VL_UNITARIO * _QTD;
	
	INSERT INTO aluguel 
	(CD_CLIENTE, CD_FUNCIONARIO, DT_RETIRADA, DT_DEVOLUCAO, VL_A_PAGAR, VL_PAGO, PAGO, FORMA_PAGAMENTO, QNT_VEZES)
	VALUES
	(_CD_CLIENTE, _CD_FUNCIONARIO, NOW(), null, null, null, 0, null, null);

	INSERT INTO aluguel_equipamento
	(CD_EQUIPAMENTO, CD_ALUGUEL, VL_ITEM, VL_UNITARIO, QTD)
	VALUES
	(_CD_EQUIPAMENTO, LAST_INSERT_ID(), @VL_TOTAL, @VL_UNITARIO, _QTD);

	UPDATE EQUIPAMENTO
	SET QTD = QTD  - _QTD
	WHERE CD_EQUIPAMENTO = _CD_EQUIPAMENTO;
END &&
DELIMITER ;

START TRANSACTION; /*Começa a transação*/
CALL SP_INSERT_ALUGUEL(3, 1, 2, 4);
COMMIT; /*Se quiser manter*/
